class Planet {
  ...
  getDisplayObject() {
    const geometry = getGeometry(this.radius, 15, 0.08)
...
const getGeometry = (radius, segments, roughness=0.05) => {
  ...
  boxGeometry.vertices.forEach(vertice => {
    ...
    const noise = (simplex.noise3D(vertice.x/radius*20, vertice.y/radius*20, vertice.z/radius*20))*radius*roughness
