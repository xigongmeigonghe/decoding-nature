...
var JonahPlanet = function () {
  function JonahPlanet(scene, radius, pos) {
    _classCallCheck(this, JonahPlanet);
  ...
  _createClass(JonahPlanet, [{
    key: "getDisplayObject",
    value: function getDisplayObject() {
      var geometry = getGeometry(this.radius, 12, 0.08);
...
var getGeometry = function getGeometry(radius, segments) {
  ...
  boxGeometry.vertices.forEach(function (vertice) {
    ...
    var noise = jonahSimplex.noise3D(vertice.x / radius * 20, vertice.y / radius * 20, vertice.z / radius * 20) * radius * roughness;
